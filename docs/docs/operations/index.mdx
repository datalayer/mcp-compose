...
sequenceDiagram
    ...
    ProcessManager->>STDIO: Close stdin + SIGTERM
    STDIO-->>ProcessManager: Exit (or SIGKILL after timeout)
    Composer->>HTTP: SIGTERM
    HTTP-->>Composer: Exit (or SIGKILL after 5 s timeout)
...
3. If the process is still running after the timeout, **SIGKILL** is sent to force termination.
...
MCP Compose registers handlers for `SIGTERM` and `SIGINT` at startup. When either signal is received, the full shutdown sequence runs automatically. This means downstream servers are cleaned up even if MCP Compose is terminated by an orchestrator (e.g., Kubernetes, Docker, systemd).
