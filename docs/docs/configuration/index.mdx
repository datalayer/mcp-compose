...
prune_downstreams_on_start = false  # Kill dangling downstreams before starting
```
...
When `prune_downstreams_on_start` is enabled, MCP Compose will scan the system process table at startup for any running processes whose command lines match the downstream servers defined in the configuration. Any matching processes are killed before new downstream servers are started.

This is useful in environments where a previous mcp-compose instance may have been terminated uncleanly (e.g. `kill -9`, OOM kill, or a host crash), leaving orphaned downstream MCP server processes still running and potentially holding ports or resources.
...
|--------|------|---------|-------------|
| `prune_downstreams_on_start` | bool | `false` | Kill dangling downstream processes from a previous run before starting new ones |

The same behaviour can be triggered on a per-invocation basis with the CLI flag `--prune-downstreams-on-start`, which overrides the config file setting:
