...
### Example 1: Local Development with Multiple STDIO Servers
...
### Example 2: Anaconda Package Server with Token Authentication

Connect to Anaconda's MCP server using token-based authentication. This pattern applies to any MCP server that requires API credentials.
...
[[servers.proxied.stdio]]
name = "anaconda"
command = ["uvx", "anaconda-mcp-server"]
...
The Anaconda MCP server provides tools for searching packages, managing environments, and interacting with Anaconda.org. The API token is passed via environment variable—set `ANACONDA_API_TOKEN` in your shell or in the MCP client configuration. The `restart_policy = "on-failure"` ensures the server recovers from transient network issues when communicating with Anaconda's API.
...
### Example 3: Team Server with HTTP Transport and API Key Protection
